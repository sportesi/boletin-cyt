				...
				chart_pie = new Highcharts.Chart({
				    ...
				    series: [{
                        ...
                        try {
                            ...
                            $query = "SELECT TT.id,CC.name,TT.total 
                            FROM (SELECT T.id,T.total FROM (
                            SELECT CN.id,CN.total FROM (
                            SELECT C.id,COUNT(*) total FROM category C, news N 
                            WHERE C.id = N.category_id AND N.user_id=" . $id . " 
                            AND C.deleted = 0 AND C.status = 1 
                            GROUP BY N.category_id ORDER BY C.Name DESC) CN UNION SELECT 
                            C.id,0 total FROM category C WHERE C.deleted = 0 AND C.status = 1) T 
                            GROUP BY T.id) TT, category CC WHERE TT.id = CC.id";
					...
					tooltip: {
						...
						data: <?php
                            try {
                                ...
                                $query = "SELECT T.id,T.total FROM (
                                SELECT CN.id,CN.total FROM (
                                SELECT C.id,COUNT(*) total FROM category C, news N 
                                WHERE C.id = N.category_id AND N.user_id=" . $id . " AND 
                                C.deleted = 0 AND C.Status = 1 GROUP BY N.category_id 
                                ORDER BY C.Name DESC) CN UNION SELECT 
                                C.id,0 total FROM category C WHERE C.deleted = 0 AND C.status = 1) 
                                T GROUP BY T.id";
